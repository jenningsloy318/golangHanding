...
在上述程序的第 6 行中, 我们试图将类型` [3]int` 的变量赋给类型为 `[5]int` 的变量，这是不允许的，因此编译器将抛出错误 <span style="color:red">main.go:6: cannot use a (type [3]int) as type [5]int in assignment</span>。
...
如果你只需要值并希望忽略索引，则可以通过用 _ 空白标识符替换索引来执行，这样 `for` 循环忽略索引。
```go
for _, v := range a { // ignores index  
}
```

但是如果只希望得到索引而忽略值，就只是用一个变量就能达到目的
```go
for i = range a { // 只是去除index
...
切片的长度是**切片中的元素数**。切片的容量是**从创建切片索引开始的底层数组中元素数**。
